...
bool Input::initialize(Engine *instance)
{
    ...
    instance->subscibe_to_event(SDL_EventType::SDL_KEYUP,
                                (eventfn)&handle_key_up);
    instance->subscibe_to_event(SDL_EventType::SDL_KEYDOWN,
                                (eventfn)&handle_key_down);

    instance->subscribe_to_post_update((updatefn)&post_update);
